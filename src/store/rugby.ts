...
export const Rugby_getGames = readable(async (id?:number,date?:string,league?: number, season?: number,team?:number,timezone?:string) => {
    try {
...
export const  Rugby_getStandings = readable(async (league: number, season: number, team?: number, stage?: string, group?: string) => {
    try {
        const response = await axios.get("https://v1.rugby.api-sports.io/standings", {
            params: {
                league,
                season,
                team,
...
export const Rugby_getAllLeagues = readable(async (id?: number, name?: string, country_id?: number, country?: string, type?: string, season?: number, search?: string) => {
    try {
